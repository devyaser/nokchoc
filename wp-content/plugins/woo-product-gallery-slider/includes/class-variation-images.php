...
if ( !class_exists( 'WPGS_Variation_images' ) ) {
	...
	class WPGS_Variation_images {
		...
		public function init_actions() {
			add_action( 'woocommerce_product_options_general_product_data', [$this, 'add_csf'], 10, 3 );
			add_action( 'woocommerce_variation_options', [$this, 'variation_image_option_field'], 10, 3 );
			add_action( 'woocommerce_save_product_variation', [$this, 'save_variation_images'], 10, 2 );
			add_filter( 'woocommerce_available_variation', [$this, 'variation_field_data'] );

			add_action( 'admin_enqueue_scripts', [$this, 'wpgs_load_custom_admin_js'] );
			add_action( 'save_post', [$this, 'product_variation_clear_transient_data'] );
		...
		function variation_image_option_field( $loop, $variation_data, $variation ) {
			...
			), get_post_meta( $variation->ID, 'wavi_value', true ), 'wavi_value' );
